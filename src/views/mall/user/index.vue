<template>
  <div class="page user">
    <buttom-menu></buttom-menu>
    <div class="page-content">
      <div class="user-userInfos">
        <div class="user-avatar">
          <div class="user-avatar-img">
            <img src="https://m.mi.com/static/img/avatar.76a75b8f17.png" />
          </div>
          <div class="user-name">
            <!-- <div>登录/注册</div> -->
            <p>zjw</p>
            <div class="user-account">17601393590</div>
          </div>
        </div>
      </div>
      <div class="user-order">
        <van-cell title="我的订单" center is-link to="cart" value-class="user-order-title" value="全部订单" />
        <van-grid :column-num="3" icon-size="25">
          <van-grid-item v-for="(orderInfo,i) in orderInfos" :key="i" :icon="orderInfo.icon" :text="orderInfo.label" />
        </van-grid> 
        <van-divider style="background-color: #f5f5f5;" />
        <div v-for="(userOrderCategory,index) in userOrderCategorys" :key="index">
          <van-cell-group>
            <van-cell v-for="(category,i) in userOrderCategory.data" :key="i" :icon="category.icon" is-link :to='category.url' :title="category.label" />
          </van-cell-group>
          <van-divider style="background-color: #f5f5f5;" />
        </div>
      </div>
    </div>
  </div>
</template>
<style lang="stylus">
.user {
  .van-grid-item__text {
    color: rgba(0, 0, 0, 0.87);
  }

  .van-divider {
    margin: 5px 0;
  }

  .van-icon__image {
    width: 24px;
    height: 24px;
  }
}
</style>
<style lang="stylus" scoped>
.user {
  .user-userInfos {
    background-repeat: no-repeat;
    background-position: center;
    padding: 16px 0;
    background-size: 100%;
    background-image: url('https://m.mi.com/static/img/bg.63c8e19851.png');
    background-color: #f37d0f;
  }

  .user-avatar-img {
    margin: 0 8px 0 16px;
    width: 48px;
    height: 48px;
    overflow: hidden;
    box-sizing: border-box;
    border-radius: 100%;
    border: 3px solid hsla(0, 0%, 100%, 0.4);
    text-align: center;
  }

  .user-avatar {
    display: flex;
    align-items: center;
  }

  .user-avatar-img img {
    width: auto;
    height: 100%;
    margin: 0 auto;
  }

  .user-name {
    color: #fff;
    font-size: 12px;
    text-align: left;
  }

  .user-avatar .user-account {
    color: hsla(0, 0%, 100%, 0.6);
  }

  .user-order-title {
    color: rgba(0, 0, 0, 0.87);
    font-size: 12px;
  }
}
</style>
<script lang="ts">
import { Component, Vue } from "vue-property-decorator"
import buttomMenu from "@/components/buttom-menu.vue"
@Component({
  name: "user",
  components: {
    buttomMenu
  }
})
export default class extends Vue {
  private orderInfos = [
    {
      label: "待付款",
      icon:
        "data:image/png;base64,"
    },
    {
      label: "待收货",
      icon:
        "data:image/png;base64,"
    },
    {
      label: "退换修",
      icon: "https://m.mi.com/static/img/nav-4.d68723895f.png"
    }
  ]
  private userOrderCategorys = [
    {
      data: [
        {
          label: "会员中心",
          icon:
            "data:image/png;base64,",
          url: ""
        },
        {
          label: "我的优惠",
          icon:
            " data:image/png;base64,",
          url: ""
        }
      ]
    },
    {
      data: [
        {
          label: "服务中心",
          icon:
            " data:image/png;base64,",
          url: ""
        },
        {
          label: "小米之家",
          icon:
            "data:image/png;base64,",
          url: ""
        }
      ]
    },
    {
      data: [
        {
          label: "我的F码",
          icon:
            "data:image/png;base64,",
          url: ""
        },
        {
          label: "礼物码兑换",
          icon: "https://m.mi.com/static/img/i-gift.ece9aa1222.png",
          url: ""
        }
      ]
    },
    {
      data: [
        {
          label: "设置",
          icon: "https://m.mi.com/static/img/i-setting.fb9625b3f2.png",
          url: ""
        }
      ]
    }
  ]
}
</script>