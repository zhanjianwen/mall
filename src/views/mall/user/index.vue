      ...
      <div class="user-userInfos">
        <div class="user-avatar">
          ...
          <div class="user-name">
            <!-- <div>登录/注册</div> -->
            <p>zjw</p>
            <div class="user-account">17601393590</div>
          </div>
        </div>
      </div>
      <div class="user-order">
        <van-cell title="我的订单" center is-link to="cart" value-class="user-order-title" value="全部订单" />
        <van-grid :column-num="3" icon-size="25" :border="false">
          <van-grid-item v-for="(orderInfo,i) in orderInfos" :key="i" :icon="orderInfo.icon" :text="orderInfo.label" />
        </van-grid> 
        <van-divider style="background-color: #f5f5f5;" />
        <div v-for="(userOrderCategory,index) in userOrderCategorys" :key="index">
          <van-cell-group>
            <van-cell v-for="(category,i) in userOrderCategory.data" :key="i" :icon="category.icon" is-link
              :to='category.url' :title="category.label" />
          </van-cell-group>
          <van-divider style="background-color: #f5f5f5;" />
        </div>
      </div>
  ...
  export default class extends Vue {
    ...
    private userOrderCategorys = [{
      ...
      },
      {
        data: [{
            label: "我的F码",
            icon: "data:image/png;base64,",
            url: "",
          },
          {
            label: "礼物码兑换",
